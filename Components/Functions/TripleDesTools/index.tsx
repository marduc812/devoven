...
export function TripleDesEncrypt() {
  ...
  const handleEncrypt = () => {
    ...
    setError('')
    try {
      const result = CryptoJS.TripleDES.encrypt(plaintext, password).toString()
      setOutput(result)
    } catch (e) {
  ...
  return (
    <Panel
      ...
      description="Encrypt text using the Triple DES (3DES) algorithm with a password. All encryption happens client-side in your browser."
      backColor="lime"
      extraElements={
...
export function TripleDesDecrypt() {
  ...
  return (
    <Panel
      ...
      description="Decrypt Triple DES (3DES) encrypted text using your password. Paste the output from the Triple DES Encrypt tool."
      backColor="lime"
      extraElements={
        <div className="flex flex-col gap-3">
