...
export function TripleDesEncrypt() {
  ...
  useEffect(() => {
    ...
    try {
      setOutput(CryptoJS.TripleDES.encrypt(plaintext, password).toString())
      setError('')
    } catch (e) {
      setOutput('')
  ...
  return (
    <Panel
      ...
      description="Encrypt text using the Triple DES (3DES) algorithm with a password. The result updates as you type. All encryption happens client-side in your browser."
      backColor="lime"
      extraElements={
...
export function TripleDesDecrypt() {
  ...
  useEffect(() => {
    ...
    try {
      const result = CryptoJS.TripleDES.decrypt(cipherInput, password).toString(CryptoJS.enc.Utf8)
      if (!result) {
        setOutput('')
  ...
  return (
    <Panel
      ...
      description="Decrypt Triple DES (3DES) encrypted text using your password. Paste the output from the Triple DES Encrypt tool and the plaintext appears as you type."
      backColor="lime"
      extraElements={
        <div className="flex flex-col gap-3">
