...
export function Rc4Encrypt() {
  ...
  const handleEncrypt = () => {
    ...
    setError('')
    try {
      const result = CryptoJS.RC4.encrypt(plaintext, password).toString()
      setOutput(result)
    } catch (e) {
  ...
  return (
    <Panel
      title="RC4 Encrypt"
      description="Encrypt text using the RC4 stream cipher with a password. All encryption happens client-side in your browser."
      backColor="lime"
      extraElements={
...
export function Rc4Decrypt() {
  ...
  const handleDecrypt = () => {
    ...
    setError('')
    try {
      const result = CryptoJS.RC4.decrypt(cipherInput, password).toString(CryptoJS.enc.Utf8)
  ...
  return (
    <Panel
      title="RC4 Decrypt"
      description="Decrypt RC4 encrypted text using your password. Paste the output from the RC4 Encrypt tool."
      backColor="lime"
      extraElements={
        <div className="flex flex-col gap-3">
