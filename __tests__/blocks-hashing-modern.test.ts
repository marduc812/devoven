...
describe('hashing-modern registration', () => {
  ...
  it('marks them all chainable and non-terminal', () => {
    for (const id of NEW_IDS) {
      expect(OPERATION_MAP[id].chainable).toBe(true);
      expect(OPERATION_MAP[id].terminal).toBeUndefined();
    }
  });
});
...
describe('hashing-modern behaviour', () => {
  ...
  it('returns a PHC string from the Argon2 block by default', () => {
    const result = run('argon2', 'password', { m: '256', t: '1' });
